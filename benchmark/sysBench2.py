...
ALERT_THRESHOLD = 0.0575456593285546  # From AirSentinelEngine.__init__
...
# -----------------------------
# STEP 1: Sort (MANDATORY)
# -----------------------------
all_windows = sorted(dataset["windows"], key=lambda x: x["timestamp"])

# -----------------------------
# STEP 2: Subsample (FAST + SAFE)
# -----------------------------
STEP = 3            # take every 3rd window (adjust for speed)
MAX_WINDOWS = 5000  # hard cap (adjust if needed)

all_windows = all_windows[::STEP][:MAX_WINDOWS]

print(f"  ✓ Total windows (after subsample): {len(all_windows)}")

# -----------------------------
# STEP 3: Grouping (UNCHANGED)
# -----------------------------
...
for window in all_windows:
    ...
    # Keep your fallback logic intact
    ssid = window.get("ssid", bssid)
...
# -----------------------------
# STEP 4: Multi-AP Analysis (UNCHANGED)
# -----------------------------
multi_ap_ssids = {
    ssid: windows
    for ssid, windows in ssid_to_windows.items()
    if len(set(w['bssid'] for w in windows)) > 1
}
